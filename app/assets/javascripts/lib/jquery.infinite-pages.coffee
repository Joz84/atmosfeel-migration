...
# Edited to be more flexible by Awea
...
(($, window) ->
  ...
  class InfinitePages
    ...
    defaults:
      ...
      alternateLoader: false # give an alternate loader
      state:
        paused:  false
        loading: false
    ...
    next: ->
      if @options.state.done
        ...
        if @options.alternateLoader
          $(@options.navSelector).eq(0).addClass('active')
          @options.alternateLoader().done(=> @_success())
        else
          $.getScript($(@options.navSelector).attr('href'))
            .done(=> @_success())
            .fail(=> @_error())
